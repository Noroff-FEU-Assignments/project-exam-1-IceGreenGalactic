document.addEventListener ("DOMContentLoaded", function () {
    document.getElementById("submitButton").addEventListener ("click", function (event){
        ...
        if (nameValue.length < 5){
            document.querySelector(`label[for="name"]`).textContent = "Name must be more than 5 characters";
        ...
        if (!emailValidate.test(emailValue)){
            document.querySelector(`label[for="email"]`).textContent = "Invalid email address";
        ...
        if (subjectValue.length < 15){
            document.querySelector(`label[for="subject"]`).textContent = "subject must be more than 15 characters ";
        ...
        if (messageValue.length < 25){
            document.querySelector(`label[for = "message"]`).textContent = "Your message must be more than 25 characters";
